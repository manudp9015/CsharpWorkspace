...
{
    public class MutualFundSimulatorUtility
    {
        ...
        public MutualFundSimulatorUtility(UserLogin userLogin, User user, MutualFundBusiness fundBusiness, Lumpsum lumpsumInvest, Sip sipInvest)
        {
        ...
        public void MainMenu()
        {
            ...
            try
            {
                if (!_fundBusiness.IsNavAlreadyUpdated())
                {
                    _fundBusiness.UpdateFundNav();
                }
                while (true)
                {
                    Console.WriteLine("\n--- Main Menu ---");
                    ...
                    if (int.TryParse(Console.ReadLine(), out int input))
                    {
                        switch (input)
                        {
                            ...
                            case 3:
                                ...
                                break;
                            case 4:
                                _fundBusiness.GetUpcomingSIPInstallments();
                                break;
                            case 5:
                                return;
                            default:
        ...
        private void UserPortfolio()
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(_fundBusiness.GetConnectionString()))
                {
                    connection.Open();
                    string query = @"
                    ...
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                decimal originalAmount = investedAmount / (1 - MutualFundSimulatorUtility.GetFundDetails()[fundName].expenseRatio / 100m);
                                if (_user.walletBalance >= originalAmount)
                                {
                                    _fundBusiness.AddMoneyToWallet(-originalAmount);
                                    _fundBusiness.SaveExpense(fundName, originalAmount - investedAmount, startDate);
                ...
                _fundBusiness.UpdateCurrentAmountsForAllInvestments();
                decimal lumpSumProfitLoss = _fundBusiness.DisplayLumpSumPortfolio();
                decimal sipProfitLoss = _fundBusiness.DisplaySIPPortfolio();
